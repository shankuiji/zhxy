...
//        接受参数并调用service完成添加或修改
    ...
    public Result deleteGrades(@ApiParam("要删除的所有grade的id的JSON集合") @RequestBody List<Integer> ids) {
    ...
    public Result deleteGrade(@ApiParam("要删除的所有grade的id的JSON集合") @RequestBody Map<String,Integer> map) {
        System.out.println(map.get("ids"));
        gradeService.removeById(map.get("ids"));
        return Result.ok();
    }
